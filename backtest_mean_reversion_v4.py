#!/usr/bin/env python3
"""
Backtest Mean Reversion Strategy V4
Wide stops + ADX filter (only trade when market is ranging, not trending).
"""
import asyncio
...
class MeanReversionBacktester:
    """Mean reversion with ADX filter."""
    ...
    def __init__(self, config, data: dict[str, Any], initial_equity: float = 10000.0):
        ...
        # ADX filter - only trade when NOT trending
        self.adx_window = 14
        self.adx_threshold = 25  # ADX < 25 = ranging market
    ...
    def _add_indicators_full(self, df):
        """Add all indicators to full dataframe at once."""
        df = df.copy()
    ...
    def _generate_signal(self, df, symbol, current_price):
        """Generate mean reversion signal with ADX filter."""
    ...
    def run(self):
        ...
        # Prepare data - add indicators once to full dataframe
        dfs = {s: self._add_indicators_full(df) for s, df in self.data.items()}
        ...
        for i in range(len(dfs[symbols[0]])):
            ...
            if not in_position:
                for sym in symbols:
                    ...
                    # Generate signal (use growing window for signal logic)
...
async def run_backtest(config_path: str, data_dir: str, output: str):
    ...
    # Run backtest
    print("Running mean reversion backtest V4 (ADX filter)...")
...
def main():
    ...
    parser.add_argument('--output', default='results/mean_reversion_report_v4.json')
