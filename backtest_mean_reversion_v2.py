#!/usr/bin/env python3
"""
Backtest Mean Reversion Strategy V2
Wider stops/targets to reduce fee impact.
"""
import asyncio
...
class MeanReversionBacktester:
    """Mean reversion with wider stops/targets."""
    ...
    def __init__(self, config, data: dict[str, Any], initial_equity: float = 10000.0):
        ...
        # Strategy parameters - WIDER NOW
        self.rsi_period = config.strategy.rsi_period
        self.rsi_oversold = 30
        self.rsi_overbought = 70
        self.bb_window = 20
        self.bb_std = 2.0
        self.stop_loss_pct = 0.02  # 2% (was 0.5%)
        self.take_profit_pct = 0.05  # 5% (was 1.5%)
    ...
    def _generate_signal(self, df, symbol, current_price):
        ...
        # Calculate stops and targets - WIDER
        if side == Side.LONG:
...
async def run_backtest(config_path: str, data_dir: str, output: str):
    ...
    # Run backtest
    print("Running mean reversion backtest V2 (wider stops)...")
...
def main():
    ...
    parser.add_argument('--output', default='results/mean_reversion_report_v2.json')
