#!/usr/bin/env python3
"""
Backtest Mean Reversion on 1h data - V2 with wider take profit (7%).
"""
import asyncio
...
class MeanReversionBacktester:
    def __init__(self, config, data: dict[str, Any], initial_equity: float = 10000.0):
        ...
        self.take_profit_pct = 0.07  # 7% instead of 5%
    ...
    def _generate_signal(self, df, symbol, current_price):
        ...
        if side == Side.LONG:
            ...
            take_profit = current_price * (1 + self.take_profit_pct)  # Fixed % target
        else:
            stop_loss = current_price * (1 + self.stop_loss_pct)
...
async def run_backtest(config_path: str, data_dir: str, output: str):
    ...
    print("Running mean reversion backtest 1h V2 (7% TP)...")
...
def main():
    ...
    parser.add_argument('--output', default='results/mean_reversion_1h_v2_report.json')
