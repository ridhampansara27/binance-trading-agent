#!/usr/bin/env python3
"""
Backtest Mean Reversion Strategy V3
Wide stops + Trend filter (only trade with the trend).
"""
import asyncio
...
class MeanReversionBacktester:
    ...
    def _add_indicators(self, df):
        """Add RSI, Bollinger Bands, and EMA50."""
        df = df.copy()
        ...
        # EMA50 for trend
        df['ema50'] = ta_trend.ema_indicator(df['close'], window=self.trend_window)
        
        return df
    
    def _generate_signal(self, df, symbol, current_price):
        """Generate mean reversion signal with trend filter."""
        ...
        in_downtrend = current_price < ema50

        side = None

        # LONG: Oversold + below lower band + UPTREND
        if rsi < self.rsi_oversold and current_price < bb_lower and in_uptrend:
            side = Side.LONG

        # SHORT: Overbought + above upper band + DOWNTREND
        elif rsi > self.rsi_overbought and current_price > bb_upper and in_downtrend:
            side = Side.SHORT
    ...
    def run(self):
        """Run backtest across all symbols/timeframes."""
...
async def run_backtest(config_path: str, data_dir: str, output: str):
    ...
    for sym in ['BTCUSDT', 'ETHUSDT']:
        for tf in ['1m', '5m']:
            file_path = data_path / f'{sym.lower()}_{tf}.parquet'
            if file_path.exists():
                key = f'{sym}_{tf}'
    ...
    # Run backtest
    print("Running mean reversion backtest V3 (trend filter)...")
...
def main():
    ...
    parser.add_argument('--output', default='results/mean_reversion_report_v3.json')
